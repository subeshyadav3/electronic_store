...
import { ShoppingBag, Zap, Award, ArrowRight, Sparkles } from "lucide-react"

const Home = () => {
  const { products, getHomeProducts, loading, setLoading, error } = useProducts()
  ...
  const getGridClasses = (count) =>
    count <= 3
      ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 justify-items-center"
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fcfcfd] selection:bg-purple-200">
      ...
      <div className="relative overflow-hidden bg-[#0a0a0c]">
        ...
        <div className="relative container mx-auto px-4 py-24 md:py-40 text-center">
          <div className="inline-flex items-center bg-white/5 border border-white/10 rounded-full px-5 py-2 mb-8 backdrop-blur-md">
            ...
            <span className="text-gray-300 text-xs md:text-sm font-medium tracking-wider uppercase">
              Next Gen Tech Hub
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto mt-16">
            {[
              ...
              <div key={i} className="group p-8 rounded-3xl border border-white/5 bg-white/[0.03] backdrop-blur-xl hover:bg-white/[0.07] transition-all duration-500">
                <div className={`bg-gradient-to-br ${f.color} to-transparent w-12 h-12 rounded-xl flex items-center justify-center mb-6 shadow-lg`}>
                  <div className="text-white w-6 h-6">{f.icon}</div>
                </div>
      ...
      {categoryKeys.map((category, idx) => (
        <div key={category} className={`py-28 ${idx % 2 === 0 ? "bg-white" : "bg-[#fafbff]"}`}>
          <div className="container mx-auto px-4">
            <div className="flex flex-col md:flex-row items-center justify-between mb-16">
              <div className="text-left mb-8 md:mb-0">
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-3xl">{categoryIcons[category.charAt(0).toUpperCase() + category.slice(1)]}</span>
                  <div className="h-px w-12 bg-gray-200"></div>
                  <span className="text-sm font-bold text-indigo-500 uppercase tracking-[0.2em]">New Arrivals</span>
                </div>
                <h2 className="text-4xl md:text-5xl font-black text-gray-900 capitalize">{category}</h2>
              </div>
            </div>
            <div className={getGridClasses((allCategories[category] || []).slice(0, 4).length)}>
              {loading && Array.from({ length: 4 }).map((_, idx) => <ProductSkeleton key={idx} />)}
              {!loading && !error && (allCategories[category] || []).slice(0, 4).map((product) => (
                <div key={product._id} className="w-full">
                  <ProductCard products={product} />
                </div>
              ))}
...
//   const getGridClasses = (count) => {
//     if (count <= 3)
//       return "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 justify-items-center";
...
//                 desc: "Top-tier brands vetted for quality.",
//                 color: "from-blue-500",
//               },
//               {
//                 icon: <Zap />,
...
//                 desc: "2-year protection on every device.",
//                 color: "from-purple-500",
//               },
//             ].map((feature, i) => (
//               <div
//                 key={i}
//                 className="group p-8 rounded-3xl border border-white/5 bg-white/[0.03] backdrop-blur-xl hover:bg-white/[0.07] transition-all duration-500"
//               >
//                 <div
//                   className={`bg-gradient-to-br ${feature.color} to-transparent w-12 h-12 rounded-xl flex items-center justify-center mb-6 shadow-lg`}
//                 >
...
//               Finding the perfect tech shouldn't be hard. We've organized everything for your
//               convenience.
//             </p>
//           </div>
...
//         <div className="container mx-auto px-4">
//           <div className="flex flex-col items-center text-center mb-16">
//             <span className="px-4 py-1 rounded-full bg-yellow-100 text-yellow-700 text-xs font-bold mb-4 uppercase tracking-tighter shadow-sm">
//               Hot Picks
//             </span>
...
//                   className="mt-4 text-indigo-600 font-semibold underline underline-offset-4"
//                 >
//                   Try Refreshing
//                 </button>
//               </div>
...
//                   </span>
//                   <div className="h-px w-12 bg-gray-200"></div>
//                   <span className="text-sm font-bold text-indigo-500 uppercase tracking-[0.2em]">
//                     New Arrivals
//                   </span>
//                 </div>
